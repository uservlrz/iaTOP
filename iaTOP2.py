...
def ler_mensagem_por_nome_arquivo(nome_arquivo, key='mensagem'):
    arquivo_completo = f"{nome_arquivo}.pkl"  # Adiciona a extensão do arquivo
...
def processar_mensagem(prompt, acao):
    ...
    # Primeiro, adiciona a mensagem do usuário ao histórico, independentemente do tipo de ação
    nova_mensagem_usuario = {'role': 'user', 'type': 'text', 'content': prompt}
    mensagens.append(nova_mensagem_usuario)

    # Depois, processa a ação específica
    if acao == "Enviar mensagem":
        ...
        if url_imagem.startswith("http"):
            # Adiciona a imagem gerada ao histórico do chat
            mensagens.append({'role': 'assistant', 'type': 'image', 'content': url_imagem})
        else:
            st.error(url_imagem)
    
    # Após processar a mensagem ou imagem, limpa o campo de entrada e atualiza o estado
...
def apagar_conversa(nome_arquivo):
    ...
    """
    try:
        # Adiciona a extensão do arquivo ao nome para formar o caminho completo
...
def pagina_principal():
    st.markdown("""
        <h1 style='text-align: left; margin-bottom: 0;'>Chat gpTOP</h1>
        <hr style='border: 2px solid red; margin-bottom: 25px; margin-top: -5px;'/>
    ...
    if submit_button:
        processar_mensagem(prompt, acao)


def melhora_legibilidade_nome(nome_arquivo):
    # Inserir espaços antes de números
    nome_melhorado = re.sub(r"(\d+)", r" \1", nome_arquivo)
    
    # Considerar CamelCase para inserção de espaços
...
def tab_conversas(tab):
    ...
    with tab.container():
        ...
        # Transforma os nomes para uma versão mais legível
        ...
        # Encontra o nome original do arquivo baseado na escolha do usuário
